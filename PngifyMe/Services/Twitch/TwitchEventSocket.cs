...
public static class TwitchEventSocket
{
    ...
    public static EventHandler<string> CustomPowerUp;
    ...
    public static async Task Start()
    {
        ...
        _eventSubWebsocketClient.UnknownEventSubNotification += UnknownEvent;

        await _eventSubWebsocketClient.ConnectAsync(new Uri(ws));
    }

    /// <summary>
    /// temporary workarounds for beta apis
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="e"></param>
    /// <returns></returns>
    private static async Task UnknownEvent(object? sender, UnknownEventSubNotificationArgs e)
    {
        Log.Debug($"Received unknown EventSub notification: {e.Payload.Subscription.Type}");
        var eventData = e.Payload.Event;
        switch (e.Payload.Subscription.Type)
        {
            case "channel.custom_power_up_redemption.add":
                var powerup = eventData.Deserialize<CustomPowerUp>();
                CustomPowerUp?.Invoke(null, powerup.custom_power_up.title);
                break;
            default:
                break;
        }
    ...
    private static async Task WebsocketConnected(object sender, TwitchLib.EventSub.Websockets.Core.EventArgs.WebsocketConnectedArgs args)
    {
        try
        {
            ...
            await Api.Api.Helix.EventSub.CreateEventSubSubscriptionAsync("channel.custom_power_up_redemption.add", "beta",
                new Dictionary<string, string>() { { "broadcaster_user_id", Api.UserId } },
                EventSubTransportMethod.Websocket, _eventSubWebsocketClient.SessionId);

            Authenticated?.Invoke(Api, Api.Auth);
            Log.Debug("Twitch auth successful, event socket connected!");
        }
        catch (Exception e)
        {
